...
function Dashboard({ token }) {
  ...
  useEffect(() => {
    ...
    fetch('http://127.0.0.1:8000/factures', { headers })
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <div style={{flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden'}}>
      <div style={{background: 'white', borderBottom: '1px solid #eee', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
        <div style={{fontSize: '15px', fontWeight: '600', color: '#1a1a1a'}}>Tableau de bord — Mai 2026</div>
      ...
      <div style={{flex: 1, overflowY: 'auto', padding: '24px'}}>
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px'}}>
          {[
            ...
            <div key={i} style={{background: 'white', borderRadius: '10px', padding: '16px', border: '1px solid #eee'}}>
              ...
              {factures.map((f, i) => (
                <tr key={i} style={{borderTop: '1px solid #f0f0f0'}}>
                  ...
                  <td style={{padding: '12px 16px'}}>
                    <span style={{background: f.statut === 'Payée' ? '#EAF3DE' : '#FAEEDA', color: f.statut === 'Payée' ? '#3B6D11' : '#854F0B', padding: '3px 8px', borderRadius: '99px', fontSize: '11px', fontWeight: '500'}}>{f.statut}</span>
                  </td>
                  <td style={{padding: '12px 16px'}}>
